...
class CorpusUpdateRequest(BaseModel):
    ...
    """ # noqa: E501
    name: Optional[StrictStr] = Field(default=None, description="New name of the corpus. Omit to keep the current name.", json_schema_extra={"examples": ["Support knowledge base"]})
    description: Optional[StrictStr] = Field(default=None, description="New description of the corpus. Omit to keep the current description.", json_schema_extra={"examples": ["Tickets, FAQs and runbooks used by the support team."]})
    metadata: Optional[Dict[str, Any]] = Field(default=None, description="New JSON metadata. When provided, **replaces** the existing metadata map; omit to keep it unchanged.", json_schema_extra={"examples": [{"owner": "support-team", "language": "fr"}]})
