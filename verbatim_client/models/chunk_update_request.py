...
class ChunkUpdateRequest(BaseModel):
    ...
    """ # noqa: E501
    pages: Optional[List[StrictInt]] = Field(default=None, description="Replacement page span: 1-based page numbers the chunk covers. Sent empty (`[]`) it clears the span, which is what a chunk belonging to no page in particular carries. Values are sorted and de-duplicated server-side, and a value below 1 is refused.", json_schema_extra={"examples": [[2, 3, 4]]})
    metadata: Optional[Dict[str, Any]] = Field(default=None, description="Replacement metadata. This **replaces** the object rather than merging into it — send the full map you want stored, and `{}` to clear it.", json_schema_extra={"examples": [{"kind": "chunk", "section": "Article 4"}]})
    body: Optional[StrictStr] = Field(default=None, description="Replacement chunk text, written to object storage. **The vector is not recomputed**: after this call the stored embedding still describes the previous text, so the chunk keeps being retrieved for the old wording and answers with the new one. That is the intended behaviour for a typo or a redaction, and the wrong tool for a rewrite — re-ingest the document for that.", json_schema_extra={"examples": ["Article 4 — The supplier shall deliver within thirty (30) days…"]})
    __properties: ClassVar[List[str]] = ["pages", "metadata", "body"]
