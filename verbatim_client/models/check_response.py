...
class CheckResponse(BaseModel):
    ...
    """ # noqa: E501
    status: Optional[StrictStr] = Field(default=None, description="`UP` when every probe succeeded, `DOWN` as soon as one failed. Mirrors the HTTP status: `UP` is returned with 200, `DOWN` with 500.", json_schema_extra={"examples": ["UP"]})
    message: Optional[StrictStr] = Field(default=None, description="Reason of the failure, prefixed by the subsystem it comes from. Absent when every probe succeeded.", json_schema_extra={"examples": ["S3: SdkClientException: Unable to execute HTTP request: Connect timed out"]})
    checks: Optional[List[CheckItem]] = Field(default=None, description="One entry per subsystem, whatever the outcome.")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "checks": [CheckItem.from_dict(_item) for _item in obj["checks"]] if obj.get("checks") is not None else None
        })
        return _obj
