...
class DocumentInitRequest(BaseModel):
    ...
    doc_create: Optional[datetime] = Field(default=None, description="Creation date of the **source** document — the file you are uploading, not the platform row (ISO-8601, UTC). Optional: omit it when you have no file metadata to read it from, and the upload instant is stored instead. The stored value is never null, so the response always carries a date; correct it later with `PATCH /v1/doc/{id}`.", alias="docCreate", json_schema_extra={"examples": ["2026-01-15T10:30:00Z"]})
    doc_update: Optional[datetime] = Field(default=None, description="Last-modified date of the **source** document (ISO-8601, UTC). Optional, with the same fallback as `docCreate` — and the two fall back independently, so sending one does not fill in the other.", alias="docUpdate", json_schema_extra={"examples": ["2026-04-01T08:00:00Z"]})
