...
class AccessTokenCreateResponse(BaseModel):
    ...
    """ # noqa: E501
    token: Optional[StrictStr] = Field(default=None, description="The opaque access token value. Pass this as the X-Access-Token header on API calls.")
    ...
    expires_at: Optional[datetime] = Field(default=None, description="Expiry timestamp (ISO-8601, UTC). Token is invalid after this date.", alias="expiresAt", json_schema_extra={"examples": ["2026-07-07T11:00:00Z"]})
    scope: Optional[List[StrictStr]] = Field(default=None, description="Permission scopes associated with the token, if any.", json_schema_extra={"examples": [["corpus:read", "doc:update"]]})
    __properties: ClassVar[List[str]] = ["token", "createdAt", "expiresAt", "scope"]
