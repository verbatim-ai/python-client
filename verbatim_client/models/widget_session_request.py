...
class WidgetSessionRequest(BaseModel):
    ...
    """ # noqa: E501
    widget_name: StrictStr = Field(description="Name of the widget", alias="widgetName", json_schema_extra={"examples": ["Corp. Website"]})
    display_name: Optional[StrictStr] = Field(default=None, description="Display name of the session (session history). When not set, a default display name is set from Widget name", alias="displayName", json_schema_extra={"examples": ["Session on BCorp. web site for user Alpha"]})
    corpus: List[UUID] = Field(description="List of Corpus UID used by the session. Check Corpus domain to learn how init a Corpus")
    __properties: ClassVar[List[str]] = ["widgetName", "displayName", "corpus"]
