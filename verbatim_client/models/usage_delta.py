...
class UsageDelta(BaseModel):
    ...
    """ # noqa: E501
    created: StrictInt = Field(description="Items whose `created_at` falls inside this bucket.", json_schema_extra={"examples": [3]})
    removed: StrictInt = Field(description="Items whose `deleted_at` falls inside this bucket (soft-deletes only — a hard delete leaves no trace).", json_schema_extra={"examples": [0]})
    __properties: ClassVar[List[str]] = ["created", "removed"]
