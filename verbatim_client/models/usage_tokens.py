...
class UsageTokens(BaseModel):
    ...
    """ # noqa: E501
    total: StrictInt = Field(description="Lifetime token count, all-time. Includes posts and documents that were since soft-deleted.", json_schema_extra={"examples": [1245000]})
    in_period: StrictInt = Field(description="Tokens produced inside the rolling window — posts (and, at organization-scope, documents) whose `created_at` falls in `[from, to)`.", alias="inPeriod", json_schema_extra={"examples": [12450]})
    __properties: ClassVar[List[str]] = ["total", "inPeriod"]
