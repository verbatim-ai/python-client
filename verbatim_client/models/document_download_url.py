...
class DocumentDownloadUrl(BaseModel):
    ...
    """ # noqa: E501
    download_url: Optional[StrictStr] = Field(default=None, description="Presigned URL to GET the document content. Single-use, time-limited.", alias="downloadUrl", json_schema_extra={"examples": ["https://s3.fr-par.scw.cloud/bucket/documents/<corpus>/src/<doc>?X-Amz-Algorithm=..."]})
    expires_at: Optional[datetime] = Field(default=None, description="Wall-clock expiration of `downloadUrl` (ISO-8601, UTC). After this, a fresh request is required.", alias="expiresAt", json_schema_extra={"examples": ["2026-05-21T10:15:00Z"]})
    filename: Optional[StrictStr] = Field(default=None, description="Original filename of the document. The client can use it for the local save name.", json_schema_extra={"examples": ["annual-report-2025.pdf"]})
    content_type: Optional[StrictStr] = Field(default=None, description="MIME content type of the document.", alias="contentType", json_schema_extra={"examples": ["application/pdf"]})
    __properties: ClassVar[List[str]] = ["downloadUrl", "expiresAt", "filename", "contentType"]
