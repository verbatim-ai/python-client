...
class UsageCount(BaseModel):
    ...
    """ # noqa: E501
    total: StrictInt = Field(description="Lifetime count, all-time. Includes soft-deleted items.", json_schema_extra={"examples": [240]})
    created: StrictInt = Field(description="Items whose `created_at` falls inside the rolling window `[from, to)`.", json_schema_extra={"examples": [28]})
    removed: StrictInt = Field(description="Items whose `deleted_at` falls inside the rolling window `[from, to)` (soft-deletes only — a hard delete leaves no trace).", json_schema_extra={"examples": [3]})
    __properties: ClassVar[List[str]] = ["total", "created", "removed"]
