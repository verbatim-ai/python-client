...
class CheckItem(BaseModel):
    ...
    """ # noqa: E501
    name: Optional[StrictStr] = Field(default=None, description="Subsystem probed.", json_schema_extra={"examples": ["S3"]})
    status: Optional[StrictStr] = Field(default=None, description="`UP` when the subsystem answered the probe, `DOWN` otherwise.", json_schema_extra={"examples": ["UP"]})
    duration_ms: Optional[StrictInt] = Field(default=None, description="Wall-clock duration of the probe, in milliseconds.", alias="durationMs", json_schema_extra={"examples": [42]})
    error: Optional[StrictStr] = Field(default=None, description="Failure reason. Absent when the probe succeeded.", json_schema_extra={"examples": ["SdkClientException: Unable to execute HTTP request: Connect timed out"]})
    __properties: ClassVar[List[str]] = ["name", "status", "durationMs", "error"]
