...
class DocumentInit(BaseModel):
    ...
    """ # noqa: E501
    document: Optional[Document] = Field(default=None, description="Created document, in AWAITING_UPLOAD status until commit is called.")
    upload_url: Optional[StrictStr] = Field(default=None, description="Presigned URL to PUT the file content to. Single-use, time-limited.", alias="uploadUrl", json_schema_extra={"examples": ["https://s3.fr-par.scw.cloud/bucket/documents/<corpus>/src/<doc>?X-Amz-Algorithm=..."]})
    expires_at: Optional[datetime] = Field(default=None, description="Wall-clock expiration of `uploadUrl` (ISO-8601, UTC). After this, a fresh init is required.", alias="expiresAt", json_schema_extra={"examples": ["2026-05-21T10:15:00Z"]})
    __properties: ClassVar[List[str]] = ["document", "uploadUrl", "expiresAt"]
